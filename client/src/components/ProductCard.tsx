    ...
    <article className="product-card">
      ...
      <div className="product-info">
        ...
        <div className="price">
          ...
          {product.oldPriceTaka ? <span className="old-price">{taka(product.oldPriceTaka)}</span> : null}
        </div>
        {!product.isInStock && <p className="stock-note">{t("outOfStock")}</p>}
        <div className="product-buttons">
          <button className="cart-btn" type="button" disabled={!product.isInStock} onClick={() => onAddCart(product.id)}>
            <ShoppingCart size={17} aria-hidden="true" /> {t("addToCart")}
          </button>
          <Link href={`/products/${product.slug}`} className="details-btn">
