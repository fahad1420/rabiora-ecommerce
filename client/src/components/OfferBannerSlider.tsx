...
export function OfferBannerSlider() {
  ...
  useEffect(() => {
    ...
    // The automatic slide interval MUST be exactly 2 seconds (2000ms)
  ...
  const isImageBanner = current.offerType === "image_banner";
  const hasMediaImage = Boolean(current.imageUrl && current.imageUrl.trim());
  ...
  return (
    ...
    >
      <div className="container">
        {isImageBanner && hasMediaImage ? (
          /* 2. IMAGE BANNER OFFER: Only uploaded full banner image is displayed */
          <div className="offer-image-banner-card">
            <a
              href={current.linkUrl || "/#products"}
              className="offer-image-banner-link"
              title={current.title}
            >
              <img
                ...
                loading="lazy"
                decoding="async"
                className="offer-image-banner-full-img"
              />
            </a>
          ...
          /* 1. TEXT OFFER: Existing text-based design, no image required */
          <div className={`offer-banner-card ${!hasMediaImage ? "has-no-media" : ""}`}>
