    ...
    <div className="page-shell">
      ...
      <main className="confirmation-page">
        <div className="confirmation-card">
          ...
          <span className="badge">{t("orderReceived")}</span>
          <h1>{t("thankYouOrder")}</h1>
          <p>{t("orderPendingCopy")}</p>
          ...
          <div className="confirmation-details">
            ...
            {order.couponCode && order.discountAmountTaka && (
              <>
                <p>
                  <span>{t("subtotal")}</span>
                  <strong>{taka(order.originalSubtotalTaka || (order.subtotalTaka + order.discountAmountTaka))}</strong>
                </p>
                <p>
                  <span style={{ color: "#16a34a" }}>Discount ({order.couponCode})</span>
                  <strong style={{ color: "#16a34a" }}>-{taka(order.discountAmountTaka)}</strong>
                </p>
              </>
            )}
            <p>
              <span>{t("deliveryCharge")}</span>
              <strong>{order.deliveryChargeTaka === 0 ? t("freeDhaka") : taka(order.deliveryChargeTaka)}</strong>
            </p>
            <p>
              <span>{t("total")}</span>
