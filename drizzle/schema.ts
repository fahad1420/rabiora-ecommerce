...
export const users = mysqlTable("users", {
  ...
  // The existing workspace uses "user" as the stored customer role. In Rabiora UI and business rules, this role is presented as "customer".
...
export const products = mysqlTable("products", {
  ...
  index("products_category_featured_idx").on(table.categoryId, table.featured),
]);
...
export const productImages = mysqlTable("product_images", {
  ...
  index("product_images_product_cover_idx").on(table.productId, table.isCover),
]);
...
export const addresses = mysqlTable("addresses", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => [index("addresses_user_default_idx").on(table.userId, table.isDefault)]);
...
export const orderStatusValues = ["pending", "confirmed", "shipped", "delivered"] as const;
export const paymentMethodValues = ["bKash", "Nagad", "Rocket", "Cash on Delivery"] as const;
